...
namespace Zoobee.Infrastructure.Parsers.Interfaces.Services.Scheduling
{
	public interface IDownloadSchedulingService
	{
		...
		Task<IEnumerable<RawPageEntity>> GetNextBatchAsync(int batchSize, CancellationToken ct);

		// Обновляет статус страницы после попытки скачивания
		Task HandleDownloadResultAsync(RawPageEntity page, DownloadResult result, CancellationToken ct);
	}
}
