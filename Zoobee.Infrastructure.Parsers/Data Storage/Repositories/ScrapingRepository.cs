	...
	public class ScrapingRepository : IScrapingRepository
	{
		...
		public async Task<List<ScrapingTask>> GetPendingTasksAsync(int batchSize, CancellationToken ct)
		{
			...
			return await _context.ScrapingTasks
				...
				.OrderByDescending(t => t.Type == ScrapingTaskType.ProductPage) // Сначала ProductPage
				.Take(batchSize)
				.ToListAsync(ct);
		}
		...
		{
			// Sanitize content: Postgres text/varchar cannot contain NUL (0x00)
			if (!string.IsNullOrEmpty(data.Content) && data.Content.IndexOf('\0') >= 0)
			{
				data.Content = data.Content.Replace("\0", string.Empty);
			}
		...
		public async Task MarkAsTransformedAsync(Guid taskId, CancellationToken ct, ScrapingTaskType? UpdatedTaskType = null)
		{
			...
			// 4. Обновляем тип страницы, если указано.

			task.Status = RawPageStatus.Pending;
			if(UpdatedTaskType.HasValue && task.Type != UpdatedTaskType)
			{
				task.Type = UpdatedTaskType.Value;
			}
