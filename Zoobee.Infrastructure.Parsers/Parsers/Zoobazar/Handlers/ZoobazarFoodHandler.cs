...
namespace Zoobee.Infrastructure.Parsers.Parsers.Zoobazar.Handlers
{
	public class ZoobazarFoodHandler : BaseZoobazarProductHandler
	{
		...
		protected override SellingSlotDto ParseSellingSlot(HtmlDocument doc, string url)
		{
			...
			dto.SellingUrl = url;
			// 1. Ищем старую цену (это и есть базовая цена DefaultSlotPrice, если она существует)
			// Селектор ищет div, у которого есть класс 'product-card__price--old'
			var oldPriceNode = root.SelectSingleNode("//div[contains(@class, 'product-card__price--old')]");
			decimal defaultPrice = ParsePrice(oldPriceNode?.InnerText);

			// 2. Ищем текущую цену (ResultPrice)
			// Ищем div с классом 'product-card__price', но ИСКЛЮЧАЕМ тот, что 'old' (чтобы не схватить старую цену первым попавшимся)
			var currentPriceNode = root.SelectSingleNode("//div[contains(@class, 'product-card__price') and not(contains(@class, 'product-card__price--old'))]");

			// Если специфичный селектор не сработал, пробуем найти просто цену (актуально, если скидки нет и класс old отсутствует)
			if (currentPriceNode == null)
			{
				currentPriceNode = root.SelectSingleNode("//div[contains(@class, 'product-card__price')]");
			}

			decimal resultPrice = ParsePrice(currentPriceNode?.InnerText);
			dto.ResultPrice = resultPrice;

			// 3. Логика расчета скидки и заполнения полей
			if (defaultPrice > 0 && defaultPrice > resultPrice)
			{
				// Если нашли старую цену и она больше текущей — значит, есть скидка
				dto.DefaultSlotPrice = defaultPrice;
				dto.Discount = resultPrice/defaultPrice;
			}
			else
			{
				// Если старой цены нет или она странная — скидки нет
				dto.DefaultSlotPrice = resultPrice; // Базовая цена равна итоговой
				dto.Discount = 0m;
			}
		...
		// Вспомогательный метод для парсинга любых "грязных" строк с ценой
		private decimal ParsePrice(string rawPrice)
		{
			if (string.IsNullOrWhiteSpace(rawPrice)) return 0m;

			// Регулярное выражение для поиска: 
			// \d+      -> одна или более цифр (целая часть)
			// ([.,]\d+)? -> опционально, либо запятая, либо точка, за которой следует одна или более цифр (дробная часть)
			var match = Regex.Match(rawPrice, @"\d+([.,]\d+)?");

			if (match.Success)
			{
				var valueString = match.Value.Replace(",", "."); // Приводим запятую к точке (например: 103,42 -> 103.42)

				// Используем InvariantCulture, чтобы гарантировать, что точка используется как десятичный разделитель
